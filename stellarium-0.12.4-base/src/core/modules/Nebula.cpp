...
void Nebula::drawLabel(StelRenderer* renderer, StelProjectorP projector, float maxMagLabel)
{
    ...
    float angle = StelApp::getInstance().getCore()->getOrientationAngle();
    renderer->drawText(TextParams(XY[0] + shift, XY[1] + shift, str).useGravity().projector(projector).angleDegrees(angle));
}
