	 ...
	 float latitude, float altitude, float temperature, float relativeHumidity)
{
	...
	for (int i=0; i<(1+skyResolutionX)*(1+skyResolutionY); ++i)
	{
        ...
        if (fabs(point.lengthSquared()-1.0) > 1e-04)
            qDebug() << "Atmosphere.cpp: lengthSq assert: " <<
                fabs(point.lengthSquared()-1.0);

        Q_ASSERT(fabs(point.lengthSquared()-1.0) < 1e-04);
...
bool Atmosphere::lazyLoadShader(StelRenderer* renderer)
{
    ...
    QFile vertexShaderFile(SailfishApp::pathTo("shaders/xyYToRGB.glsl").toLocalFile());
