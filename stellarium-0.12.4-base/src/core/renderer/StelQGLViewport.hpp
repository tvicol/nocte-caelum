...
public:
	...
	void init(const bool npot, const QString& glVendor, const QString& glRenderer)
	{
        ...
        qDebug() << "FBO Supported " << fboSupported;
		invariant();
	}
	...
	QImage screenshot() const 
	{
        ...
        qWarning() << "Screenshot not supported.";
        return QImage();
	...
	void startFrame()
	{
		...
		if (useFBO())
		{
            ...
            // Stellarium-sailfish: no need for backBufferPainter
    ...
    bool getFboSupported() const { return fboSupported; }

    void setFboDisabled(bool v) { fboDisabled = v; }
