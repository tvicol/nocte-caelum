...
public:
        ...
        , skipFrameFbo(0)
	{
        ...
        createRenderQuadProgram();

	}

    void setQGLContext(QGLContext* context)
    {
        glContext = context;
        gl = QGLFunctions(glContext);
    }
    ...
    void setMinFps(qreal m) { minFps = m; }
    qreal getMinFps() const { return minFps; }

    void setSkipNumFrames(unsigned int n) { skipNumFrames = n; currentSkipFrameNum = 0; }
    unsigned int getSkipNumFrames() const { return skipNumFrames; }
...
protected:
    ...
    unsigned int skipNumFrames;
    unsigned int currentSkipFrameNum;
    QOpenGLFramebufferObject* skipFrameFbo;
    QOpenGLShaderProgram* renderQuadProgram;
    QOpenGLBuffer* renderQuadVbo;
    void createRenderQuadProgram();
};
