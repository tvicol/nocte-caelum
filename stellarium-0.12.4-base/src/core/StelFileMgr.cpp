...
void StelFileMgr::init()
{
    ...
    userDir = QDir::homePath() + "/.stellarium-sf";
    ...
    const QStringList& l = QStandardPaths::standardLocations(QStandardPaths::PicturesLocation);
    if (l.isEmpty())
    {
        screenshotDir.clear();
    }
...
{
    const QStringList& l = QStandardPaths::standardLocations(QStandardPaths::DesktopLocation);

    if (l.isEmpty() || !QFileInfo(l.first()).isDir())
	{
...
QString StelFileMgr::getInstallationDir()
{
    ...
    QString install_dir = SailfishApp::pathTo(".").toLocalFile();
    QFileInfo installLocation(install_dir);
    QFileInfo checkFile(install_dir + "/" + CHECK_FILE);
#endif
...
{
    const QStringList& cachePath = QStandardPaths::standardLocations(QStandardPaths::CacheLocation);
	if (cachePath.isEmpty())
	{
		return getUserDir()+"/cache";
	}
    return cachePath.first();
}
