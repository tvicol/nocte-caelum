	...
	switch (event->button())
	{
		...
		case Qt::LeftButton :
			...
			else
			{
                ...
                else if (pinchActive || hasPinched)
                {
                    pinchActive = false;
                    hasPinched = false;
                    event->accept();
...
bool StelMovementMgr::handleTouch(QTouchEvent* touchEvent)
{
    //qDebug() << "touch event";
    ...
    if (touchPoints.count() == 2) {
        ...
        //qDebug() << "Two touch event";

        if (touchEvent->touchPointStates() & Qt::TouchPointReleased)
        {
            //qDebug() << "2-touch release";
            if (pinchActive)
                hasPinched = true;
        }
        else if (touchEvent->touchPointStates() & Qt::TouchPointPressed)
        {
            //qDebug() << "2-touch begin";

            fovAtPinchBegin = getAimFov();
            touch0 = touchPoint0.pos();
            touch1 = touchPoint1.pos();

            // finish dragging if any
        ...
        else
        {
            ...
            qreal currentScaleFactor =
                    ...
                    / QLineF(touch0, touch1).length();

            zoomTo(fovAtPinchBegin / currentScaleFactor, 0.);
            //qDebug() << "Scale: " << currentScaleFactor;
        }
