...
QSettings* StelApp::s_confSettings = 0;
...
void StelApp::setConfig(QSettings* conf)
{
    StelApp::s_confSettings = conf;
}

QSettings* StelApp::getConfig()
{
    return StelApp::s_confSettings;
}
...
void StelApp::init(StelRenderer* renderer)
{
    confSettings = StelApp::s_confSettings;
...
// Handle touch events
void StelApp::handleTouch(QTouchEvent* event)
{
    // Send the event to every StelModule
    foreach (StelModule* i, moduleMgr->getCallOrders(StelModule::ActionHandleMouseMoves))
    {
        if (i->handleTouch(event))
            return;
    }
}
